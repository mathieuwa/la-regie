...
<!-- Copie d'export généricisée de agents/creative-strategist.md du plugin
     claude-ads (marketplace agricidaniel-claude-ads). Ne se met pas à jour
     toute seule : à réaligner manuellement si l'agent source du plugin évolue.
     Références réécrites vers knowledge/ads-references/ (relatif au dépôt
     exporté) au lieu du chemin utilisateur de l'installation d'origine, qui
     ne fait pas partie du paquet exporté. -->
...
[Reads knowledge/ads-references/brand-dna-template.md for schema]
[Reads knowledge/ads-references/benchmarks.md for platform copy guidance]
...
3. **Read reference files** (load on-demand, chemins relatifs au dépôt exporté) :
   - `knowledge/ads-references/brand-dna-template.md`: for voice axis interpretation
   - `knowledge/ads-references/benchmarks.md`: for platform copy benchmarks
   - `knowledge/ads-references/voice-to-style.md`: map brand voice axes to visual style attributes
   - `knowledge/ads-references/copy-frameworks.md`: select appropriate ad copy framework
   - Playbook e-commerce (`skills/ads-plan/assets/ecommerce-creative.md` dans l'installation d'origine du plugin) : hors du paquet exporté, PAS vendorisé. Si l'installation courante n'a pas ce fichier (cas normal en export), passer l'étape 7 ci-dessous et généraliser le concept sans playbook dédié plutôt que d'échouer.
...
7. **E-Commerce Playbook** (when business type is e-commerce, ET SEULEMENT si le fichier existe dans l'installation) :
   - Read `skills/ads-plan/assets/ecommerce-creative.md` s'il est présent
   ...
   - Absent (cas normal en export) : ignorer cette étape, ne pas la simuler
